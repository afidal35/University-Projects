...
bump() {
  ...
  if [ "$latest_commit" = "$head_commit" ]; then
    ...
    echo "Latest found : $latest_ver"
    export VERSION=$next_ver
    echo "Next version $VERSION"
    export VERSION_PROPERTIES="app/src/main/resources/version.properties"
    rm -f $VERSION_PROPERTIES
    echo "Bumping next version"
    echo "version=$VERSION" >> $VERSION_PROPERTIES
    echo "Committing version.properties"
    git add $VERSION_PROPERTIES
    git commit -m "Bump version to $VERSION"
    echo "Creating tag"
    git tag -a "$VERSION" -m "Auto-Release"
    echo "Pushing to repository"
    git push another-tag-origin "$VERSION"
  fi
}
